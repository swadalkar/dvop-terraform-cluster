        ...
        script{
         withCredentials([azureServicePrincipal('610320c7-4515-4551-9505-0e3f4b7cd21f')]) {
           ...
           docker.image('dvopsimages.azurecr.io/base/dvopssupport').inside("-v ${WORKSPACE}:/mnt -u -0:0") {
              ...
              sh "cd /mnt/cluster; terraform init -backend-config=tenant_id=$AZURE_TENANT_ID -backend-config=client_id=$AZURE_CLIENT_ID -backend-config=client_secret=$AZURE_CLIENT_SECRET -backend-config=subscription_id=a80b3931-1be9-4942-afb4-2cf9a987a136"
