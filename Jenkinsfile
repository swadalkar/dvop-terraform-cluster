        ...
        script{
         withCredentials([azureServicePrincipal('610320c7-4515-4551-9505-0e3f4b7cd21f')]) {
           ...
           docker.image('dvopsimages.azurecr.io/base/dvopssupport').inside("-v ${WORKSPACE}:/mnt -u -0:0") {
              ...
              sh "cd /mnt/cluster; terraform init '-backend-config=key=${CLUSTER_NAME}' -var clientid=$AZURE_CLIENT_ID -var clustersecrect=$AZURE_CLIENT_SECRET -var tenantid=$AZURE_TENANT_ID -var subscriptionid=a80b3931-1be9-4942-afb4-2cf9a987a136"
